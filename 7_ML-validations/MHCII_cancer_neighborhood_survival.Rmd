...
cancers.dat <- cancers.dat %>% mutate(MHCII.bin = ifelse(MHCII > (median(MHCII) + sd(MHCII)),"MHCII.high","MHCII.low"),
...
for (t in therapies){
  ...
  a <- df.set %>% select(class, MHCII.high.prop) %>%
                  unique() %>% arrange(factor(class, levels = order.m.profiles)) %>% 
                  pull(MHCII.high.prop)
  ...
  p <- ggplot(df.set, aes(x=class, y=Neighbors)) + geom_point(aes(col=log2FC, size=FDR.cat)) + 
          scale_colour_gradient2(low= "blue", mid="grey90", high ="red", midpoint = 0, limits = c(-1.7,3.15)) +
  ...
  print(p)
  ggsave(p, filename=paste0(out.folder.name, "Neighboors_MHCIIhighVSlow_FDRdot",t,"_30px.svg"),
...
#Calculating median and sd of MHCII in cancer cells
MHCII.med.value <- median(log2(cancers.dat$MHCII))
MHCII.sd.value <- sd(log2(cancers.dat$MHCII))

#Get the proportion of cancer cells per core, with MHCII expression above the med+sd
MHCII.prop.cores <- NULL
for (t in therapies){
  sel.df <- cancers.dat %>%  filter(therapy_sequence2 == {t})
  MHCII.prop.core <- sel.df %>% group_by(cycif.slide, cycif.core.id) %>%  
                      summarise(Prop.pos=sum(log2(MHCII) > (MHCII.med.value + MHCII.sd.value))/n()) 
  MHCII.prop.cores <- rbind(MHCII.prop.cores, MHCII.prop.core)
}
...
MHCII.cores.prop.clin <- MHCII.cores.prop.clin %>% group_by(patient) %>% mutate(N.cores=n()) %>% filter(N.cores >= 1)
...
MHCII.cores.prop.clin %>% filter(cycif.slide=="TMA_43_616") %>% arrange(desc(Prop.pos))
```
...
```{r}
formulas.cox <- c('Surv(timelastfu, finalstatus) ~ MHCII.strat + Age + Stage + Molecular.profile2 + therapy_sequence2 + resdx',
            'Surv(daystoprogression, progression)~ MHCII.strat + Stage + Molecular.profile2 + therapy_sequence2 + resdx')
...
formulas.cox <- c('Surv(timelastfu, finalstatus) ~  MHCII.strat + Age + Stage + resdx',
              'Surv(daystoprogression, progression)~ MHCII.strat +  Stage + resdx')
...
for (t in therapies){
  df <- MHCII.prop.patient.clin %>% filter(therapy_sequence2 == {t},
  ...
  for (i in 1:length(formulas.kapplan)){
     ...
     print(p1)
     ggsave(p1, file=paste0(out.folder.name, "Cox_MHCII_others",t, "_", formulas.names[i],".png"),
...
formulas.cox <- c('Surv(timelastfu, finalstatus) ~ MHCII.strat + Age + Stage + therapy_sequence2 + resdx',
            'Surv(daystoprogression, progression)~ MHCII.strat + Stage + therapy_sequence2 + resdx')

df <- MHCII.prop.patient.clin %>% filter(Molecular.profile2=="Other")
df$MHCII.strat <- ntile(df$Med.prop,3)
...
for (i in 1:length(formulas.cox)){
  ...
  cox.model <- coxph(as.formula(formulas.cox[i]), data=df)
  p1 <- ggforest(cox.model, data = df, main=paste0(formulas.labs[i]," ALL"))
  ggsave(p1, file=paste0(out.folder.name, "Cox_MHCII_others_", formulas.names[i],".png"),
       width = 14, height = 11, units = "cm")
}
```
...
selected.markers <- c("Ki67", "pSTAT1", "pTBK1",  "TIM3", "PD1",  "MHCII")
...
for (marker in selected.markers){
  ...
  for (t in therapies){
      ...
      sel.dat[,cols] <- outlier.trim(sel.dat[,cols])
      sel.dat <- sel.dat %>%
  ...
  for (t in therapies){
    ...
    print(p)
     ggsave(p, filename=paste0(out.folder.name, paste0("Expression_", marker, "_MHCIIhighVSlow_FDRdot_",t,".png")),
             height = 4.7, width = 3.7, units = "in")
  }
}
```
...
```{r}
df.sel <- Cancer.neighbors.clin[Cancer.neighbors.clin$Molecular.profile2 == "BRCAloss",]


vars <- c("Ki67","PD1", "PDL1_488", "pSTAT1", "GranzymeB")
for (t in c("PDS","IDS")){
  df <- df.sel[which(df.sel$therapy_sequence2 == t),]
  for (v in vars){
    aux <- df
    aux[,v] <- log2(aux[,v])
    p <- ggplot(aux, aes_string(x="GlobalCellType", y=v, fill="Cancer.neighbor")) +
              geom_boxplot(aes(fill=Cancer.neighbor)) + theme_bw() +
               labs(title=paste0(t,"_", v))
    print(p)
  }
}
```
