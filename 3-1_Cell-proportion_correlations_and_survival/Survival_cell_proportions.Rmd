...
#patient.cell.prop <- read.table(file="/home/fernpere/NKI/Analysis_results/01_Cell-proportions_Heatmaps2/tCycif_cell-proportions-patients.csv", sep=",", header=TRUE)

core.cell.prop <- read.table(file="/home/fernpere/NKI/Analysis_results/01_Survival_Proportions_November_medians/Proportions_by_core.csv", sep=",", header = TRUE)

#Reading clinical info
core.patient.slide <- read.table(file="/home/fernpere/NKI/Clinical_intersection/All-slides_cycif2samples.txt",
                                 header = TRUE, sep="\t")

#Reading clinical info
clinical.info <- read.table(file="/home/fernpere/NKI/Clinical_data/T-CycIF.updated.csv", sep=",", header = TRUE)

Mol.profiles.patients <- read.table(file="/home/fernpere/NKI/Tables/Molecular_profiles_patients_20231208.csv",
            sep=",", header=TRUE)

#out.put.folder <- "/home/fernpere/NKI/Analysis_results/01_Survival_Proportions_November_min3cores/"
out.put.folder <- "/home/fernpere/NKI/Analysis_results/01_Survival_Proportions_November_medians/"
...
```{r}
output.folder = "/home/fernpere/NKI/Analysis_results/01_Survival_Proportions_November_medians/"
#output.folder <- "/home/fernpere/NKI/Analysis_results/01_Survival_Proportions_November_min3cores/"
...
for (index in 1:length(formulas)){
  for (c in c("IntraCutoff", "GlobalCutoff")){
    for (t in therapies){
      ...
      ggplot2::ggsave(filename = paste0(output.folder, "Forest_plot_REDUCED2_multivarCox_", formula.suffix[index], "_",
                                        c, "_", t, ".png"),
  ...
  ggplot2::ggsave(filename = paste0(output.folder, "Forest_plot_REDUCED2_multivarCox_", formula.suffix[index], ".png"), plot = p,
...
c.index.iter <- function(input.dat, prop.training=0.8, formula="Surv(timelastfu, finalstatus) ~ ."){
    dt <- sort(sample(nrow(input.dat), nrow(input.dat)*.80)) #Selecting rows for training and validation set
    dat.train <- input.dat[dt,] #Training set
    dat.val <- input.dat[-dt,] #Validation set
    #Creating model
    model <- coxph(as.formula(formula), data=dat.train)
    predicted_valid <- predict(model, newdata = dat.val, type = "survival")
...
lymphocites2 <- c("T.regs", "CD4.PD1.T.cells", "CD11c.MY", "CD15.MY", "B.cells")
...
vars.name <- c("complete", "reduced", "reduced2")
vars.to.test <- list(lymphocites, lymphocites2, lymphocites3, lymphocites4)
for (m in 1:4){
  ...
  for (t in "PDS"){
    if (t == "All"){
      dat <- patient.cell.prop.clin2
      dat <- dat %>% select(c("Molecular.profile2", "figo_stage2", "refage2", "therapy_sequence",
  ...
  c.indexes <- sapply(1:400, function(x){c.index.iter(dat)})
...
```{r}
cols.fun= circlize::colorRamp2(c(0, 1), c("blue","yellow"))

for (t in therapies){
  dat <- patient.cell.prop.clin2 %>% filter(therapy_sequence == {{t}}) %>% select(lymphocites)
  hmap <- Heatmap(cor(dat), name="Correlation", column_title=t, col = cols.fun)
  png(paste0(output.folder, "Heatmap_cellprop_correlations_",t,".png"), units="cm", width=16, height=14, res=800)
  draw(hmap)
  dev.off()
}

dat <- patient.cell.prop.clin2 %>% select(lymphocites)
hmap <- Heatmap(cor(dat), name="Correlation", column_title="All cohort", col = cols.fun)
png(paste0(output.folder, "Heatmap_cellprop_correlations_all.png"), units="cm", width=16, height=14, res=800)
draw(hmap)
dev.off()
```
