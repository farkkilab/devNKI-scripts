...
setwd("D:/users/fperez/NKI_TMAs_AF/QCs3/")
dir.create("plots", showWarnings = FALSE)
dir.create("gated", showWarnings = FALSE)


# List all the data
files <- list.files( "D:/users/fperez/NKI_TMAs_AF/QCs3/", full.names = T)
files <- files[grepl(pattern = "filter", x=files)]

# Hopefully you don't have to edit this regex
names(files) <- gsub('[.].*','',gsub('filter_','',gsub(".*/(.*).*", "\\1", files)))
registerDoFuture()
plan(multiprocess)
...
#code by Fernando:
data <- read.csv(files, sep=',', stringsAsFactors = F, header=TRUE)
names(data) <- names(files)
...
plot_qc_plot(labeled.old,labeled) # This is to compare two different results. If you have only one result, just call the function plot_qc_plot(labeled,labeled)
plot_umaps(labeled)