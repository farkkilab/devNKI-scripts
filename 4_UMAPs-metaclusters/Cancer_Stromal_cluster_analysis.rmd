...
tmpdir.stromal <- "D:/users/fperez/NKI_TMAs_AF/Analysis_results/02_MetaClusters_November/SOM_consensusClusters_stromal2/"

som.consensus.cancer <- som.metaclustering.consensus(mat.cancer, tempdir=tmpdir.cancer)
som.consensus.stromal <- som.metaclustering.consensus(mat.stromal, tempdir=tmpdir.stromal)
```
...
consensus.stromal <- som.consensus.stromal[[1]]
som.stromal <- som.consensus.stromal[[2]]
consensus.stromal.k <- consensus.stromal[[k.stromal]]$consensusClass

#Extraction of metaclusters ids per cell
metalabels.stromal <- consensus.stromal.k[som.stromal$map$mapping[,1]]
...
#Saving heatmaps with Z-score plus min-max normalization
pdf(file= paste0(outout.folder.name,"/Metaclusters_Stromal_Heatmap_MinMax.pdf"), height = 8, width = 10)
...
write.table(file="D:/users/fperez/NKI_TMAs_AF/Tables/All_cells_scimap-format_columns-reduced_20231129.csv", aux1, sep=",",
            row.names=FALSE)
...
write.table(file=paste0(outout.folder.name, "Metaclusters_abundance_per_core_20231129.csv"), big.data.set.subtypes.numbers,
            sep=",", row.names=FALSE)
```
