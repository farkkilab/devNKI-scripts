...
# List all CSV files
# ===================================================
files <- list.files(folder_path, pattern = "CD3plus\\.csv$", full.names = TRUE)
...
for (marker in markers) {
  ...
  #   labs(title = paste(marker, "histogram (CD3+ cells, patients pooled)"),
  #        x = marker, y = "Count") +
  #   theme_minimal()
  # print(p_hist)
  ...
  p_violin <- ggplot(marker_data, aes_string(x = "Condition", y = marker, fill = "Condition")) +
    ...
    labs(title = paste(marker, "distribution in CD3+ cells (patients pooled)"),
  ...
  p_box <- ggplot(marker_data, aes_string(x = "Condition", y = marker)) +
    ...
    labs(title = paste(marker, "levels in CD3+ cells (Patients pooled)"), y = marker) +
    theme_minimal()
  
  # ---- SAVE PLOT ----
  plot_title <- paste(marker, "levels in CD3+ cells (Patients pooled)")
  ...
  for (marker in markers) {
   ...
   print(p_violin)
    ...
    p_box <- ggplot(patient_data, aes_string(x = "Condition", y = marker)) +
      ...
      labs(title = paste0(marker, " levels in CD3+ cells (Patient ", patient_data$Patient_display[1], ")"), y = marker) +
      theme_minimal()
    
    # ---- SAVE PLOT ----
    plot_title <- paste0(marker, " levels in CD3+ cells (Patient ", patient_data$Patient_display[1], ")")
    file_name <- paste0(gsub("[^A-Za-z0-9_\\-]", "_", plot_title), ".png")
    file_path <- file.path("E:/F_Revision_Exp/Main_Blocking_Exp/Plots/Statistics Plots pvalue", file_name)
    png(filename = file_path, width = 500, height = 500)
    print(p_box)
