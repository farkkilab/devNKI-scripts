...
```{r}
neighbors.counts <- read.table(file="D:/users/fperez/NKI_TMAs_AF/Tables/Spatial-cell-proportions_radious46px_metaclusters_20240112.csv",
                               sep=",", header = TRUE)
```
...
formulas.cox <- c('Surv(timelastfu, finalstatus) ~ MHCII.strat + Age + Stage + Molecular.profile + Therapy.sequence',
            'Surv(daystoprogression, progression)~ MHCII.strat + Stage + Molecular.profile + Therapy.sequence')
...
formulas.cox <- c('Surv(timelastfu, finalstatus) ~ MHCII.strat + Age + Stage + therapy_sequence2',
            'Surv(daystoprogression, progression)~ MHCII.strat + Stage + therapy_sequence2')
